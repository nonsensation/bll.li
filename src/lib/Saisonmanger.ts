    ...
    prepareGames = async (team: TeamInfo): Promise<GameCardInfo[]> =>
    {
        ...
        for (const leagueId of leagueIds) {
            ...
            const filteredGames = games.filter(
                (g) => g.home_team_name === this.teamName || g.guest_team_name === this.teamName,
            );
            ...
            if (this.upcomingGamesCount > 0) upcomingGames = upcomingGames.slice(0, this.upcomingGamesCount);

            if (this.finishedGamesCount > 0) finishedGames = finishedGames.slice(0, this.finishedGamesCount);
            ...
            for (const game of [...upcomingGames, ...liveGames, ...finishedGames]) {
                ...
                const gameCard = {
                    ...
                    leagueShortName: team.leagueName,
